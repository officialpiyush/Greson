...
const { promptArgument } = require("../utils/ArgumentUtil");

const CHANNEL_REGEXP = Argument.regex.channel;

function resolveChannel(query, guild) {
    if (query instanceof Channel)
        return guild.channels.has(query.id) ? query : null;
    if (query instanceof Message)
        return query.guild.id === guild.id ? query.channel : null;
    if (typeof query === "string" && CHANNEL_REGEXP.test(query))
        return guild.channels.get(CHANNEL_REGEXP.exec(query)[1]);
    return null;
}
...
class ChannelName extends Argument {
    async run(arg, possible, msg) {
        ...
        if (!arg || !arg.length)
            throw `${
                possible.name
            } Must be a valid name, id or channel mention`;
        const results = [];
        ...
        if (results.length > 0) {
            ...
            const filtered = results.filter(channel =>
                regWord.test(channel.name)
            );
        ...
        default:
            return promptArgument(msg, querySearch.slice(0, 10));
        }
    }
}
